...
/**
 * TODO: give props to other guys for this...
 */
abstract class dao_model {

    public static function tablename($alias = '') {
        $name = static::get_model_tablename();
        // $name = sprintf('enrol_oes_%s', get_called_class() . 's');
    ...
    public function save() {
        ...
        // if we're saving an object for the first time,
        if ( ! isset($this->id)) {
            // be sure to set the appropriate timestamps
        ...
        try {
            // retrieve the freshly persisted record, throwing exception if none
            $record = $DB->get_record($tablename, ['id' => $this->id], '*', MUST_EXIST);
            
            // return a model
            return self::upgrade($record);
